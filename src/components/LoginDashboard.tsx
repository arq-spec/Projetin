...
import { Freelancer, UserProfile, SystemUser, RegistrationRequest } from '../types';
import { 
  Shield, 
  Briefcase, 
  Users, 
...
export default function LoginDashboard({ freelancers, users, onUpdateUsers, onAddRegistrationRequest, onLogin }: LoginDashboardProps) {
  // Login input states
  const [loginInput, setLoginInput] = useState('');
  const [passwordInput, setPasswordInput] = useState('');
  
  // View states
  const [step, setStep] = useState<'identify' | 'password' | 'set-password' | 'register'>('identify');
  ...
  // Normalization for comparing CPF
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check if user is typing numbers
    if (/^\d/.test(val)) {
  ...
  const handleIdentify = (e: React.FormEvent) => {
    ...
    if (!input) {
      setErrorMsg('Por favor, digite seu Usuário ou CPF.');
      return;
    }

    // Search by username (case insensitive) or by unformatted/formatted CPF
    const matched = users.find(u => {
      const matchUsername = u.username.toLowerCase() === input.toLowerCase();
      const matchCpf = cleanCPF(u.cpf) === cleanCPF(input) && cleanCPF(input) !== '';
      return matchUsername || matchCpf;
    });

    if (!matched) {
      setErrorMsg('Usuário ou CPF não encontrado no banco de dados corporativo.');
      return;
    }

    if (matched.statusAprovacao === 'Pendente') {
      setErrorMsg('Seu cadastro está em análise pela administração. Aguarde aprovação para acessar.');
      return;
    }

    if (matched.statusAprovacao === 'Rejeitado') {
      setErrorMsg('Seu cadastro foi rejeitado. Entre em contato com a administração.');
      return;
    }
  ...
  const handleLoginWithPassword = (e: React.FormEvent) => {
    ...
    if (passwordInput === foundUser.password) {
      onLogin({
        ...
        freelancerId: foundUser.freelancerId,
  ...
  const handleSetNewPassword = (e: React.FormEvent) => {
    ...
    if (newPassword.length < 4) {
      setErrorMsg('A senha precisa ter no mínimo 4 caracteres para sua segurança.');
      return;
    }
    ...
    // Success! Update password in users database list
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    if (!regNome || !regSobrenome || !regCpf || !regEmail || !regCargo || !regWhatsapp) {
      setErrorMsg('Preencha os campos obrigatórios.');
      return;
    }

    const cpfNorm = cleanCPF(regCpf);
    ...
    if (users.some(u => cleanCPF(u.cpf) === cpfNorm)) {
    ...
    const request: RegistrationRequest = {
      ...
      siteReferencia: regWebsite,
      dataSolicitacao: new Date().toISOString()
    };

    onAddRegistrationRequest(request);
  ...
  const getRoleIcon = (role: string, className = "w-4 h-4") => {
    switch (role) {
      ...
      case 'Freelancer':
        return <Users className={`${className} text-emerald-500`} />;
      default:
      ...
      <div className="w-full max-w-[1280px] mx-auto bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] flex overflow-hidden border border-white/60 relative z-10 min-h-[700px]">
        ...
        <div className="w-full lg:w-[55%] flex flex-col p-8 sm:p-12 relative overflow-y-auto">
          ...
          <div className="flex justify-between items-center mb-12 sm:mb-20">
            <div className="font-black text-2xl tracking-tighter uppercase text-neutral-900 flex items-center gap-3">
                  ...
                  <form onSubmit={handleIdentify} className="space-y-5">
                    <div className="space-y-2">
                      <input
                        ...
                        className="w-full p-4 border border-neutral-200/60 bg-white/50 backdrop-blur-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition-all font-medium text-sm placeholder:text-neutral-400"
                        autoFocus
                      />
                    </div>
                    ...
                    <div className="pt-2">
                      <button
                        type="submit"
                        className="w-full py-4 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl text-sm transition-all shadow-lg shadow-purple-600/20 cursor-pointer"
                      >
                        Continuar
                      </button>
                    </div>
                  ...
                  <form onSubmit={handleLoginWithPassword} className="space-y-5">
                    <div className="space-y-2 relative">
                      <input
                        ...
                        className="w-full p-4 border border-neutral-200/60 bg-white/50 backdrop-blur-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition-all font-medium text-sm placeholder:text-neutral-400 pr-12"
                        autoFocus
                      />
                      <button
                        type="button"
